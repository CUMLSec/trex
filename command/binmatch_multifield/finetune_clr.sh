...
BMT_PATH=checkpoints/binmatch_multifield_fine_tune_seen_clr
mkdir -p $BMT_PATH
cp checkpoints/data_noisy_mmod_meaningful_masking/checkpoint_11_43000_15_final.pt $BMT_PATH/checkpoint_pretrain.pt

python train.py data-bin/binmatch_clr_multifield \
  --max-positions 1024 \
  --max-sentences $MAX_SENTENCES \
  --user-dir custom_tasks \
  --task binmatch_clr_multifield \
  --reset-optimizer --reset-dataloader --reset-meters \
  --required-batch-size-multiple 1 \
  --arch roberta_multifield \
  --criterion binmatch_clr_multifield \
  ...
  --truncate-sequence \
  --last-layer -1 \
  --restore-file $BMT_PATH/checkpoint_pretrain.pt | tee result/binmatch_fine_tune_seen_clr
