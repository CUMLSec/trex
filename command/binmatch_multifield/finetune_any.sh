...
BMT_PATH=checkpoints/binmatch_multifield_fine_tune_unseen_meaningful_masking
mkdir -p $BMT_PATH
cp checkpoints/data_noisy_mmod_meaningful_masking/checkpoint_3_9000.pt $BMT_PATH/checkpoint_pretrain.pt
...
python train.py data-bin/binmatch_multifield \
  ...
  --truncate-sequence \
  --last-layer -1 \
  --restore-file $BMT_PATH/checkpoint_pretrain.pt \
  --num-classes 2 | tee result/binmatch_fine_tune_9000_meaningful_masking
