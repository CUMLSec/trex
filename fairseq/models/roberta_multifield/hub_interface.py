...
class RobertaHubInterface(nn.Module):
    """A simple PyTorch Hub interface to Reformer.
    ...
    def encode(self, sentence: dict, *addl_sentences, no_separator=False) -> torch.LongTensor:
        """
        ...
        requires leading spaces. For example::
            ...
            [0, 8331, 2]
        """
        assert set(sentence.keys()) == set(configs.fields)
        ...
        for field in configs.fields:
            full_sentence = '<s> ' + sentence[field] + ' </s>'
            tokens[field] = self.task.source_dictionary[field].encode_line(full_sentence, append_eos=False,
                                                                           add_if_not_exist=False)
            tokens[field] = tokens[field].long()
        return tokens

    def decode(self, tokens: Dict[str, torch.LongTensor]):
        assert tokens[configs.fields[0]].dim() == 1
        sentences = {}

        for field in configs.fields:
            tokens[field] = tokens[field].numpy()
            if tokens[field][0] == self.task.source_dictionary[field].bos():
                tokens[field] = tokens[field][1:]  # remove <s>
            eos_mask = (tokens[field] == self.task.source_dictionary[field].eos())
            doc_mask = eos_mask[1:] & eos_mask[:-1]
            sentences[field] = np.split(tokens[field], doc_mask.nonzero()[0] + 1)
            # sentences = [self.bpe.decode(self.task.source_dictionary.string(s)) for s in sentences]
            sentences[field] = [self.task.source_dictionary[field].string(s) for s in sentences]
            if len(sentences[field]) == 1:
                sentences[field] = sentences[field][0]
        return sentences

    def extract_features(self, tokens: Dict[str, torch.LongTensor], return_all_hiddens: bool = False,
                         last_layer=-1) -> torch.Tensor:
        for field in configs.fields:
            if tokens[field].dim() == 1:
                tokens[field] = tokens.unsqueeze(0)
    ...
    def predict(self, head: str, tokens: Dict[str, torch.LongTensor], return_logits: bool = False,
                return_all_hiddens=False, last_layer=-1):
        features = self.extract_features({field: tokens[field].to(device=self.device) for field in configs.fields},
                                         return_all_hiddens=return_all_hiddens, last_layer=last_layer)
