...
class RobertaModel(FairseqLanguageModel):
    ...
    def register_classification_head(self, name, num_classes=None, inner_dim=None, **kwargs):
        ...
        # self.classification_heads[name] = RobertaClassificationHeadAdvanced(
        #     self.args.encoder_embed_dim,
        #     inner_dim or self.args.encoder_embed_dim,
        #     num_classes,
        #     self.args.pooler_activation_fn,
...
class RobertaClassificationHeadAdvanced(nn.Module):
    """Head for sentence-level classification tasks."""

    def __init__(self, input_dim, inner_dim, num_classes, activation_fn, pooler_dropout):
        super().__init__()
        self.dense1 = nn.Linear(input_dim, inner_dim)
        self.dense2 = nn.Linear(inner_dim, inner_dim * 4)
        self.activation_fn = utils.get_activation_fn(activation_fn)
        self.dropout = nn.Dropout(p=pooler_dropout)
        self.out_proj = nn.Linear(inner_dim * 4, num_classes)
...
class RobertaEncoder(FairseqDecoder):
    ...
    def forward(self, src_tokens, features_only=False, return_all_hiddens=False, masked_tokens=None,
                freeze_encoder=False, freeze_embedding=False, **unused):
        """
        Args:
        ...
        if freeze_embedding:
            self.freeze_module_params(self.sentence_encoder.embed_tokens)
            self.freeze_module_params(self.sentence_encoder.segment_embeddings)
            self.freeze_module_params(self.sentence_encoder.embed_positions)
