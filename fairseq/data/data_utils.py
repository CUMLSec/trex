...
def collate_tokens_multiple(values, pad_idx, eos_idx=None, left_pad=False, move_eos_to_beginning=False, multiple=128):
    """Convert a list of 1d tensors into a padded 2d tensor."""
    size = max(v.size(0) for v in values)

    # round to nearest to multiple
    size = multiple * math.ceil(size / multiple)
    res = values[0].new(len(values), size).fill_(pad_idx)
