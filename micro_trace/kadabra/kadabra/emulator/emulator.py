...
class Emulator:
    def __init__(self, arch_id, md=None):
        ...
        # print instruction by taking capstone md
        self.md = md
        self.trace = []
        self.static = []
    ...
    def start_execution(self, start, end, count=0, timeout=0):
        self.start_addr = start
        self.end_addr = end
        self.mu.emu_start(start, end, count=count, timeout=timeout)
    ...
    def set_hooks(self, mem_rw=False, mem_unmapped=False, basic_block=False, instruction=False):
        ...
        if mem_unmapped and not self.hooks[HOOK_MEM_UNMAPPED]:
            h = self.mu.hook_add(UC_HOOK_MEM_READ_UNMAPPED | UC_HOOK_MEM_WRITE_UNMAPPED,
    ...
    def initialise_regs_random(self, value=None):
        for reg in self.registers:
            ...
            if not value:
                ...
                self.reg_write(reg, getrandbits(self.reg_size(reg) - 2))
            else:
